...
class GlobalListingsTests(unittest.TestCase):
    def test_renders_global_sources_and_priority_connectors(self) -> None:
        ...
        self.assertIn("Live official feeds: `8`", report)
        self.assertIn("Verified snapshots: `0`", report)
        self.assertIn("Priority connectors: `1`", report)
        self.assertIn("Restricted research-only markets: `1`", report)
        self.assertIn("Beyond Wall Street", page)
        self.assertIn("1 priority; 0 planned", page)
        self.assertIn("Documented only; no ingestion", page)
        self.assertIn("HKEX, LSE, ASX, TSX, JPX, SGX, CVM and ESMA have official ingestion paths", page)
        ...
        self.assertIn('href="cvm-monitor.html"', page)
        self.assertIn('href="esma-monitor.html"', page)
