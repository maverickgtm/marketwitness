...
CAPABILITIES = (
    ...
    {
        "key": "market_context_workspace",
        "title": "Cross-Asset Markets",
        "status": "public_source_no_key",
        "cost": "No data fee",
        "provider_ids": ("treasury-yield-curve",),
        "output": "Official Treasury curve-regime explorer plus attributed TradingView crypto, commodity, FX and benchmark views",
        "route": "/dashboard/market-context",
        "limitation": "Only Treasury observations are stored; widget values remain display-only and no output recommends a trade.",
    },
    {
...
def render_open_edition_report(snapshot: dict[str, Any]) -> str:
    lines = [
        ...
        "",
        "| Capability | Availability | Cost | Open View | Output | Limitation |",
        "|---|---|---|---|---|---|",
    ]
    for item in snapshot["capabilities"]:
        lines.append(
            f"| {item['title']} | `{item['status']}` | {item['cost']} | "
            f"[Open view]({item['route']}) | "
...
def render_open_edition_html(snapshot: dict[str, Any]) -> str:
    capability_cards = "".join(
        ...
        f'<a href="{escape(item["route"])}">Open view</a>'
        "</article>"
        for item in snapshot["capabilities"]
    )
    modes = "".join(
