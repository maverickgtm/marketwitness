...
def fetch_edinet_offering_filings(
    ...
    key = (api_key or os.environ.get("MARKETWITNESS_EDINET_API_KEY", "")).strip()
    if not key:
        raise EdinetDataError(
            "Live EDINET collection requires --api-key or MARKETWITNESS_EDINET_API_KEY."
        )
    query = urlencode(
...
<title>MarketWitness | EDINET Offering Filing Watch</title><style>
