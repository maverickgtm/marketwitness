...
def fetch_opendart_equity_filings(
    ...
    key = (api_key or os.environ.get("MARKETWITNESS_OPENDART_API_KEY", "")).strip()
    if not key:
        raise OpenDartDataError(
            "Live OpenDART collection requires --api-key or MARKETWITNESS_OPENDART_API_KEY."
        )
...
<title>MarketWitness | Korea OpenDART Equity Offering Watch</title><style>
...
</style></head><body><header><nav><a href="/dashboard/global-listings">Global Listings Watch</a> / OpenDART</nav>
