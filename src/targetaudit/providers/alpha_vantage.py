...
def configured_api_key(api_key: str | None = None) -> str:
    candidate = (api_key or os.environ.get("TARGETAUDIT_ALPHA_VANTAGE_API_KEY", "")).strip()
    ...
    if not candidate:
        raise AlphaVantageDataError(
            "Alpha Vantage live requests require TARGETAUDIT_ALPHA_VANTAGE_API_KEY "
...
def fetch_adjusted_daily(
    ...
    request = Request(
        ...
        headers={"User-Agent": "TargetAudit/0.1 market-research-adapter", "Accept": "application/json"},
    )
    try:
...
def render_prices_report(imported: AlphaVantageImport, as_of: date) -> str:
    ...
    return "\n".join(
        [
            ...
            "and an adjusted close. TargetAudit derives adjusted high and adjusted low",
...
<title>TargetAudit | Adjusted Prices</title><style>
...
</style></head><body><header><nav>TargetAudit / Financials Scorecard / Price Evidence</nav>
...
<main><p class="notice">The endpoint returns raw high/low plus adjusted close. TargetAudit scales intraday bounds by adjusted close / raw close. Alpha Vantage currently marks this daily-adjusted API as premium; public output rights remain under review.</p>
