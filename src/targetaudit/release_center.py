...
from .storage import read_evaluations
...
def build_release_decision(
    ...
    release_ready = source_ready and run_ready and lineage_ready
    return {
...
<title>TargetAudit | Release Center</title><style>
:root{{--bg:#071016;--panel:#0f1c24;--line:#20343d;--text:#edf1ef;--muted:#98abb0;--mint:#56daac;--gold:#f0bc62;--blue:#62a6ff;--red:#ff7d72;}}
*{{box-sizing:border-box}}body{{margin:0;background:var(--bg);color:var(--text);font:15px/1.5 Inter,Arial,sans-serif}}header,main{{max-width:1160px;margin:auto;padding:30px 28px}}nav,.meta{{color:var(--muted);text-transform:uppercase;letter-spacing:.08em;font-size:13px}}h1{{font-size:clamp(36px,5vw,56px);line-height:1.06;margin:38px 0 14px}}.lead{{color:var(--muted);font-size:17px;max-width:850px}}.cards{{display:grid;grid-template-columns:repeat(4,1fr);gap:16px;margin:35px 0}}.card,.notice,.panel,.table-wrap{{background:var(--panel);border:1px solid var(--line);border-radius:14px}}.card{{padding:18px 20px}}.card p{{margin:0;color:var(--muted)}}.card strong{{font-size:34px;color:var(--mint);display:block}}.blocked{{color:var(--red)!important}}.notice{{border-left:3px solid var(--gold);color:var(--muted);padding:15px 18px}}.panel{{padding:18px 22px;margin:20px 0}}ul{{margin:8px 0 0;padding-left:20px}}table{{width:100%;border-collapse:collapse}}th,td{{padding:15px;border-bottom:1px solid var(--line);text-align:left;vertical-align:top}}th{{text-transform:uppercase;font-size:12px;color:var(--muted);font-weight:500}}.pill{{display:inline-block;border-radius:999px;padding:5px 9px;font-size:12px}}.public_ready{{color:var(--mint);background:rgba(86,218,172,.12)}}.internal_only{{color:var(--blue);background:rgba(98,166,255,.12)}}.integration_pending,.missing_source{{color:var(--gold);background:rgba(240,188,98,.12)}}.table-wrap{{overflow:hidden;margin-top:18px}}@media(max-width:820px){{.cards{{grid-template-columns:repeat(2,1fr)}}}}
</style></head><body><header><nav>TargetAudit / Financials / Release Center</nav><h1>Release only<br>what is defensible.</h1>
...
<section class="cards"><article class="card"><p>Release status</p><strong class="{state_class}">{escape(snapshot['release_status'])}</strong></article><article class="card"><p>Source gate</p><strong class="{'blocked' if snapshot['source_gate_status'] != 'pass' else ''}">{escape(snapshot['source_gate_status'])}</strong></article><article class="card"><p>Lineage gate</p><strong class="{'blocked' if snapshot['lineage_gate_status'] != 'pass' else ''}">{escape(snapshot['lineage_gate_status'])}</strong></article><article class="card"><p>Run quality</p><strong class="{'blocked' if snapshot['quality_gate_status'] != 'pass' else ''}">{escape(snapshot['quality_gate_status'])}</strong></article></section></header>
<main><p class="notice">{escape(snapshot['publication_note'])}</p><section class="panel"><h2>Decision Blockers</h2><ul>{blocker_rows}</ul></section><h2>Required Source Controls</h2><div class="table-wrap"><table><thead><tr><th>Control</th><th>Status</th><th>Blocking Reason</th></tr></thead><tbody>{controls}</tbody></table></div></main></body></html>"""
